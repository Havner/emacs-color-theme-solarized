    ...
    (let ((bg-back   '(:background back))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                    ...
                    (normal
                     (if window-system
                         `((diff-added (,@fmt-bold ,@fg-green))
                           ...
                           (diff-removed (,@fmt-bold ,@fg-red))
                           (diff-refine-change (,@fmt-bold ,@fg-blue)))
                       `((diff-added (,@fg-green))
                         (diff-changed (,@fg-yellow))
                         (diff-removed (,@fg-red))
                         (diff-refine-change (,@fg-blue))))))
                ...
                ;; bm visual bookmarks
                (bm-face (,@bg-orange ,@fg-base03))
                (bm-persistent-face (,@bg-blue ,@fg-base03))
                ...
                (helm-buffer-file (,@fg-cyan))
                (helm-buffer-modified (:inherit helm-buffer-file ,@fmt-ital))
                (helm-buffer-not-saved (,@fg-orange))
                (helm-buffer-process (,@fg-green))
                (helm-buffer-saved-out (,@fmt-revr ,@fg-red ,@bg-back))
                (helm-buffer-size (,@fg-base01))
                (helm-candidate-number (,@fmt-bold ,@bg-base02 ,@fg-base1))
                (helm-ff-dotted-directory (,@fg-base00))
                (helm-ff-dotted-symlink-directory (,@fg-base00))
                ...
                (helm-M-x-key (,@fmt-none ,@fg-magenta))
                (helm-match (,@fg-blue ,@fmt-undr))
                (helm-moccur-buffer (,@fmt-undr ,@fg-cyan))
                (helm-selection (,@bg-base02))
                (helm-selection-line (,@fg-red ,@fmt-revr))
                (helm-separator (,@fg-red))
                (helm-source-header (:inherit helm-header))
                (helm-visible-mark (,@fmt-bold ,@bg-back ,@fg-magenta))
                (helm-header (,@fg-orange))
                (helm-source-header (,@fg-magenta ,@fmt-revr))
                (helm-header-line-left-margin (,@fg-yellow ,@fmt-revr))
                ;; markdown
                ...
                (company-tooltip-annotation (,@fg-yellow))
                ...
                ;; magit next branch
                (magit-branch-current (,@fg-blue))
                (magit-branch-local (,@fg-violet))
                ...
                (magit-diff-file-heading (,@fg-cyan))
                (magit-diff-file-heading-highlight (,@fg-cyan :inherit highlight))
                ...
                ;; ediff
                (ediff-odd-diff-Ancestor (,@bg-base02 :inherit diff-changed))
                (ediff-odd-diff-A (,@bg-base02 :inherit diff-removed))
                (ediff-odd-diff-B (,@bg-base02 :inherit diff-added))
                (ediff-odd-diff-C (,@bg-base02 :inherit diff-changed))
                (ediff-even-diff-Ancestor (,@bg-base02 :inherit diff-changed))
                (ediff-even-diff-A (,@bg-base02 :inherit diff-removed))
                (ediff-even-diff-B (,@bg-base02 :inherit diff-added))
                (ediff-even-diff-C (,@bg-base02 :inherit diff-changed))
                (ediff-current-diff-Ancestor (:inherit diff-changed))
                (ediff-current-diff-A (:inherit diff-removed))
                (ediff-current-diff-B (:inherit diff-added))
                (ediff-current-diff-C (:inherit diff-changed))
                (ediff-fine-diff-Ancestor (,@fmt-revr :inherit diff-changed))
                (ediff-fine-diff-A (,@fmt-revr :inherit diff-removed))
                (ediff-fine-diff-B (,@fmt-revr :inherit diff-added))
                (ediff-fine-diff-C (,@fmt-revr :inherit diff-changed))
                ;; smerge
                (smerge-markers (:inherit highlight))
                (smerge-other (:inherit diff-added))
                (smerge-base (:inherit diff-changed))
                (smerge-mine (:inherit diff-removed))
                (smerge-refined-changed (,@fmt-revr))
                (smerge-refined-added (,@fmt-revr :inherit diff-added))
                (smerge-refined-removed (,@fmt-revr :inherit diff-removed))
                ;; rpm
                (rpm-spec-dir-face (,@fg-green))
                (rpm-spec-doc-face (,@fg-magenta))
                (rpm-spec-ghost-face (,fg-red))
                (rpm-spec-macro-face (,fg-yellow))
                (rpm-spec-obsolete-tag-face (,@fmt-revr ,@fg-red))
                (rpm-spec-package-face (,@fg-red))
                (rpm-spec-section-face (,@fg-yellow))
                (rpm-spec-tag-face (,@fg-blue))
                (rpm-spec-var-face (,@fg-cyan))
                ;; e2wm
                (e2wm:face-history-list-normal (:inherit default))
                (e2wm:face-history-list-select1 (,@fg-red))
                (e2wm:face-history-list-select2 (,@fg-blue))
                (e2wm:face-subtitle (,@fg-base2))
                (e2wm:face-title (,@fg-yellow))
                ;; minesweeper
                (minesweeper-0 (,@fg-base01))
                (minesweeper-blank (,@fg-base01))
                (minesweeper-explode (,@fmt-revr ,@fg-red))
                (minesweeper-marked (,@fmt-revr ,@fg-green))
                (minesweeper-mismarked (,@fmt-revr ,@fg-yellow))
                (minesweeper-neighbor (:inherit highlight))
                ;; avy
                (avy-lead-face (,@fmt-bold ,@bg-base02 ,@fg-red))
                (avy-lead-face-0 (,@fmt-bold ,@bg-base02 ,@fg-cyan))
                (avy-lead-face-1 (,@fmt-bold ,@bg-base02 ,@fg-red))
                (avy-lead-face-2 (,@fmt-bold ,@bg-base02 ,@fg-cyan))
                )))))
